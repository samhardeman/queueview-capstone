...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchLocations() {
      try {
        ...
        await Promise.all(locations.map(async (location, index) => {
          try {
            const hoursResponse = await fetch(`/api/hours/${location.name}`);
    ...
    async function showLocationDetails(location) {
      ...
      try {
        ...
        const detailsResponse = await fetch(`/api/location/${location.name}`);
        ...
        if (details.image) {
          try {
            const imageResponse = await fetch(`/api/image/${details.image}`);
            const imageData = await imageResponse.json();
            html += `
              <div class="image-container">
                <img src="${imageData.image_url}" alt="${location.name}" />
              </div>
            `;
          } catch (imageError) {
